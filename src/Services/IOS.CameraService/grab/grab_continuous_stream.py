...
class ContinuousStreamDemo:
    ...
    def __init__(self, camera_ip="192.168.10.5", save_dir="./data", save_interval=10):
        self.camera_ip = camera_ip
        self.save_dir = save_dir
        self.save_interval = save_interval  # 每隔多少帧保存一次
        self.camera = None
        self.frame_count = 0
        self.save_enabled = True  # 默认开启自动保存
        
        # 确保保存目录存在
        self._ensure_directories()
        
        print(f"自动保存已启用，每隔 {self.save_interval} 帧保存一次")
    ...
    def _should_save_frame(self):
        """判断是否应该保存当前帧"""
        return self.save_enabled and (self.frame_count % self.save_interval == 0)
        
    def demo_generator_mode(self):
        """演示生成器模式的连续流"""
        print("=== 生成器模式连续流演示 ===")
        print("按 'q' 键退出，按 'p' 键暂停/恢复自动保存")
            ...
            with QtVisionSick(ipAddr=self.camera_ip) as camera:
                ...
                for success, depth_data, intensity_image, camera_params in camera.get_continuous_stream(max_frames=1000):
                    if success:
                        ...
                        # 显示保存状态
                        save_status = "AUTO SAVE ON" if self.save_enabled else "AUTO SAVE OFF"
                        cv2.putText(display_image, save_status, (10, 60), 
                                  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0) if self.save_enabled else (0, 0, 255), 2)
                        cv2.putText(display_image, f"Save Interval: {self.save_interval}", (10, 90), 
                        ...
                        if key == ord('q'):
                            ...
                            print(f"自动保存: {'开启' if self.save_enabled else '关闭'}")
    ...
    def demo_performance_test(self):
        ...
        try:
            with QtVisionSick(ipAddr=self.camera_ip) as camera:
                ...
                print(f"保存帧数: {self.frame_count // self.save_interval}")
    ...
    def _save_frame(self, depth_data, intensity_image, camera_params):
        ...
        print(f"自动保存帧 {self.frame_count}: {intensity_filename}")
...
def main():
    ...
    parser.add_argument("--save-interval", type=int, default=10, help="自动保存间隔帧数 (默认: 10)")
    
    args = parser.parse_args()
    
    demo = ContinuousStreamDemo(camera_ip=args.ip, save_dir=args.dir, save_interval=args.save_interval)
    
    print("=== 西克相机连续流演示程序 ===")
    print(f"相机IP: {args.ip}")
    print(f"保存目录: {args.dir}")
    print(f"演示模式: {args.mode}")
    print(f"自动保存间隔: 每 {args.save_interval} 帧保存一次")
    print("-" * 50)
