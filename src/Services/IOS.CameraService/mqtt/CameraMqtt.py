...
if __name__ == "__main__":
    ...
    # 定义消息回调函数
    def on_camera_command(topic: str, data: Any, msg):
        print(f"收到相机命令: {topic} -> {data}")
    
    def on_connect(client, userdata, flags, rc):
        print("MQTT连接成功，开始订阅主题")
    
    # 使用MQTT客户端
    with CameraMqtt(config) as mqtt_client:
        # 设置连接回调
        mqtt_client.set_on_connect_callback(on_connect)
        
        # 订阅主题
        mqtt_client.subscribe("camera/+/command", callback=on_camera_command)
        
        # 发布测试消息
        mqtt_client.publish_camera_data("camera_001", "depth", [1.2, 1.5, 2.0])
